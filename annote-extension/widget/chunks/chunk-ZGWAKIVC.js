...
// lib/sttVocabulary.ts
var MAX_STT_PROMPT_CHARS = 600;
var MAX_ELEMENT_NAMES = 8;
var MAX_NAME_CHARS = 40;
var BASE_UI_VOCABULARY = "Feedback about a web page UI. Terms: padding, margin, font size, font weight, pixels, px, rem, hex color, hashtag F F zero zero zero zero, CTA, call to action, modal, tooltip, dropdown, placeholder, hover state, focus ring, breadcrumb, viewport, z-index, border radius, box shadow, contrast, alignment, whitespace, accordion, carousel, checkbox, toggle, nav bar, class name, btn-primary.";
function extractQuotedNames(listText) {
  if (!listText) return [];
  const names = [];
  const re = /"([^"]+)"/g;
  let match;
...
function buildSttVocabularyPrompt(context) {
  const names = [];
  if (context) {
  ...
  const prompt = unique.length > 0 ? `${BASE_UI_VOCABULARY} On-page names: ${unique.map((n) => `"${n}"`).join(", ")}.` : BASE_UI_VOCABULARY;
...
lucide-react/dist/esm/lucide-react.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-ZGWAKIVC.js.map
