...
export type GlobalRailVariant = "rail" | "drawer";

export interface GlobalRailContentProps {
  /** Visual variant. "rail" = desktop sidebar; "drawer" = mobile expanded drawer. */
  variant?: GlobalRailVariant;
  /** Called after a nav item is clicked. Used by the drawer to auto-close. */
  onItemClick?: () => void;
  /** Desktop-only: forces the rail into the expanded state. */
  forceExpanded?: boolean;
  /** Desktop-only: width class applied when the rail is expanded. */
  expandedWidthClass?: string;
  /** Desktop-only: close handler when forceExpanded is true. */
  onForceClose?: () => void;
}

export default function GlobalRailContent({
  variant = "rail",
  onItemClick,
  forceExpanded = false,
  expandedWidthClass = "w-[248px]",
  onForceClose,
}: GlobalRailContentProps = {}) {
  ...
  if (isSessionPage && !forceExpanded && !isDrawer) return null;

  // Drawer is always fully expanded; no collapse toggle.
  const effectiveCollapsed = isDrawer ? false : (forceExpanded ? false : isCollapsed);

  return (
    <>
    <div
      className={cn(
        "relative z-30 group flex flex-col min-h-0 shrink-0",
        isDrawer ? "h-full w-full" : "h-full"
      )}
      ref={sidebarRef}
    >
      <aside
        className={cn(
          "relative flex flex-col bg-[var(--surface)] h-full shrink-0 min-h-0 overflow-visible py-4",
          isDrawer ? "w-full" : "rounded-[var(--content-card-radius)]",
          !isDrawer && mounted && "transition-[width] duration-300 ease-[cubic-bezier(0.4,0,0.2,1)]",
          isDrawer
            ? "items-stretch"
            : effectiveCollapsed
              ? "w-[64px] items-stretch group/rail"
              : cn(expandedWidthClass, "items-stretch")
        )}
        ...
        >
          {effectiveCollapsed ? (
            ...
            <div className="flex items-center justify-between w-full py-2">
              <Link href="/dashboard" className="flex items-center pl-1.5" aria-label="Annote home" onClick={onItemClick}>
                <Image
      ...
      {workspacePopoverOpen && (
        <div
          className={cn(
            ...
            isDrawer ? "left-3" : effectiveCollapsed ? "left-[64px]" : "left-[248px]"
          )}
          role="dialog"
        ...
        >
            ...
            <>
              ...
              {isLoadingWorkspaces ? (
                ...
                allWorkspaces.map((ws) => {
                  const isActiveWs = (activeWorkspaceId ?? workspaceId) === ws.workspaceId;
          ...
          <div className="flex flex-col gap-1">
            <a
              ...
              onClick={() => { setWorkspacePopoverOpen(false); onItemClick?.(); }}
