    ...
    <div className="h-full border-r border-gray-100 flex flex-col">
      <div className="pt-5 px-3">
        ...
        <div className="flex flex-col">
          ...
          {loadMoreRef && <div ref={loadMoreRef} style={{ height: "1px" }} aria-hidden />}
          {loadingMore && (
            <div className="text-[14px] text-secondary py-4 text-center">Loading…</div>
          )}
          {!hasMore && feedback.length > 0 && (
            <div className="text-[12px] text-meta py-4 text-center">No more feedback</div>
          )}
          {hasReachedLimit && feedback.length > 0 && (
            <div className="shrink-0 py-1.5 text-[12px] text-meta text-center">Reached maximum items</div>
          )}
        </div>
      </div>
    </div>
