...
const ROLE_GRID_CLASS =
  "relative flex flex-col items-center justify-center gap-2 h-[90px] rounded-xl border border-[var(--border)] bg-white/60 backdrop-blur-sm cursor-pointer transition-all duration-150 hover:bg-white/80 hover:border-[var(--brand)] hover:scale-[1.02] active:scale-[0.97]";
const SIZE_OPTION_CLASS =
  "relative rounded-xl bg-white/60 border border-[var(--border)] backdrop-blur-sm py-4 text-center cursor-pointer transition-all duration-150 hover:bg-white/80 hover:border-[var(--brand)] hover:scale-[1.02] active:scale-[0.97]";
...
const inputClass =
  "w-full h-11 rounded-xl bg-white/70 border border-[var(--border)] px-4 text-[var(--text-heading)] placeholder:text-[var(--text-placeholder)] backdrop-blur-sm focus:outline-none focus:border-[var(--brand)] focus:ring-[3px] focus:ring-[var(--brand)]/20 transition-all duration-150 ease-out";
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col space-y-6">
        ...
        <div ref={roleContainerRef} className="relative grid grid-cols-3 gap-4 mt-4">
          ...
          {ROLES.map(({ id, icon: Icon }) => (
            ...
            >
              ...
              <span className={`text-sm font-medium ${role === id ? "text-[var(--brand-hover)]" : "text-[var(--text-body)]"}`}>{id}</span>
            </button>
          ))}
        </div>
      </div>
        ...
        <div ref={sizeContainerRef} className="relative grid grid-cols-4 gap-3 mt-4">
          ...
          {COMPANY_SIZES.map(({ value, number, label }) => {
            ...
            return (
              ...
              >
                <span className={`block text-lg font-semibold ${isSelected ? "text-[var(--brand-hover)]" : "text-[var(--text-body)]"}`}>{number}</span>
      ...
      <button
        ...
        className="h-12 w-full rounded-xl font-semibold text-white text-[16px] transition-all duration-150 ease-out disabled:opacity-50 flex items-center justify-center mt-8 hover:scale-[1.02] hover:shadow-[0_14px_40px_rgba(23,117,224,0.35)] active:scale-[0.97]"
        style={{
