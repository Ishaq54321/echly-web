          ...
          <div key={step.label} className="flex items-center">
            <span
              className={`block shrink-0 rounded-full transition-all duration-150 ease-out ${
                isActive ? "w-3 h-3 bg-[var(--brand)] shadow-[0_0_12px_rgba(70,110,255,0.45)]" : "w-3 h-3 bg-[var(--border)]"
              }`}
              aria-current={isActive ? "step" : undefined}
            />
            {!isLast && (
              <div className="h-[2px] w-20 bg-[var(--surface-hover)] shrink-0 mx-0" aria-hidden />
            )}
          </div>
        );
      })}
