...
function Trend({ direction }: { direction: "up" | "flat" | "down" }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 text-[12px] ${className}`}>
  ...
  return (
    ...
    >
      ...
      <div className="p-4 grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div>
          <p className="text-[12px] font-medium uppercase tracking-wider text-[var(--text-tertiary)]">
            Resolution velocity
          </p>
          <p className="mt-1">
        ...
        {avgTime ? (
          <div>
            ...
            <p className="mt-1 text-[14px] font-medium text-[var(--text-primary-strong)] tabular-nums">
              {avgTime}
            </p>
          </div>
        ) : null}
        ...
        <div>
          ...
          <ul className="mt-1 space-y-0.5">
            {momentum.ownerLoadBalance.slice(0, 3).map((o) => (
              <li
                key={o.ownerId}
                className="text-[12px] text-[var(--text-secondary-soft)] flex justify-between gap-2"
              >
