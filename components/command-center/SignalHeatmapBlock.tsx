          ...
          <ul className="space-y-2">
            {buckets.slice(0, 12).map((b) => (
              <li key={b.label} className="flex items-center gap-3">
                <span className="w-32 shrink-0 text-[12px] text-[var(--text-secondary-soft)] truncate">
                  {b.label}
                </span>
                ...
                <span className="w-8 text-right text-[12px] tabular-nums text-[var(--text-tertiary)]">
                  {b.count}
                </span>
              </li>
            ))}
