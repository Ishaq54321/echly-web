...
export async function signEmailVerifiedToken(uid: string): Promise<string> {
...
): Promise<EmailVerifiedTokenPayload | null> {
...
export function buildEmailVerifiedCookieString(token: string): string {
  const attrs = [
    `${COOKIE_NAME}=${token}`,
    "Path=/",
    "HttpOnly",
...
export const EMAIL_VERIFIED_COOKIE_NAME = COOKIE_NAME;
