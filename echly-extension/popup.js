...
<rect width="24" height="24" fill="white"/>
  ...
  // node_modules/@firebase/auth/dist/web-extension-esm/register-7238101c.js
...
${this.customData.serverResponse}`;
  ...
  async function signInWithGoogle() {
    ...
    const authUrl = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${clientId}&response_type=id_token&redirect_uri=${encodeURIComponent(redirectUri)}&scope=openid%20email%20profile&nonce=${crypto.randomUUID()}`;
    return new Promise((resolve, reject) => {
      chrome.identity.launchWebAuthFlow(
        ...
        async (responseUrl) => {
          ...
          const idToken = url.hash.substring(1).split("&").find((param) => param.startsWith("id_token="))?.split("=")[1];
          if (!idToken) {
  ...
  // echly-extension/src/screenshotUpload.ts
  async function uploadScreenshot(imageDataUrl, sessionId, feedbackId) {
  ...
  function setSessionListState(state, message) {
  ...
  function updateCaptureButtonState() {
    const hasSelection = sessionSelect.style.display === "block" && sessionSelect.value !== "" && sessionSelect.value !== "__none";
  ...
  function bindFormSubmit() {
    formEl.addEventListener("submit", async (e) => {
      e.preventDefault();
      const sessionId = sessionSelect.style.display === "block" ? sessionSelect.value === "__none" ? "" : sessionSelect.value : "";
  ...
  function init() {
    ...
    signInBtn = getEl("signInBtn");
    signOutBtn = getEl("signOutBtn");
    authSection = getEl("authSection");
    formSection = getEl("formSection");
    sessionSelect = getEl("sessionSelect");
    sessionSelectState = getEl("sessionSelectState");
    captureBtn.disabled = true;
