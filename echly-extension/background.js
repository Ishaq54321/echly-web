...
${this.customData.serverResponse}`;
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
      ...
      chrome.identity.launchWebAuthFlow(
        ...
        async (responseUrl) => {
          ...
          try {
            ...
            const idToken = url.hash.substring(1).split("&").find((p) => p.startsWith("id_token="))?.split("=")[1] ?? null;
            if (!idToken) {
    ...
    if (request.type === "ECHLY_UPLOAD_SCREENSHOT") {
      const { imageDataUrl, sessionId, feedbackId } = request;
      const timestamp = Date.now();
      const path = `sessions/${sessionId}/feedback/${feedbackId}/${timestamp}.png`;
      const screenshotRef = ref(storage, path);
      uploadString(screenshotRef, imageDataUrl, "data_url", { contentType: "image/png" }).then(() => getDownloadURL(screenshotRef)).then((url) => sendResponse({ url })).catch((err) => sendResponse({ error: String(err) }));
      return true;
    }
    ...
    if (request.type === "ECHLY_PROCESS_FEEDBACK") {
      let normalizePriority2 = function(s) {
        ...
        return "medium";
      }, generateFeedbackId2 = function() {
        return crypto.randomUUID?.() ?? `fb-${Date.now()}-${Math.random().toString(36).slice(2, 11)}`;
      };
      var normalizePriority = normalizePriority2, generateFeedbackId = generateFeedbackId2;
      ...
      (async () => {
        try {
          ...
          if (screenshot) {
            ...
            const firstFeedbackId = generateFeedbackId2();
